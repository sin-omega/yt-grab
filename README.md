...
- **Age/sign-in gated videos.** Some videos return `"Video is login
  required"` from the default client. The download route now retries with
  `TV_EMBEDDED`, `IOS`, and `ANDROID` client types in turn, which generally
  don't enforce that gate. If `ClientType.TV_EMBEDDED` (etc.) ever errors on
  import, `youtubei.js` may have renamed its client-type enum — check the
  current values in `node_modules/youtubei.js` or its README and update
  `CLIENT_FALLBACKS` in `app/api/download/route.js` accordingly.
